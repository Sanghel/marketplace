...
import CustomizedSlider from '@/app/components/Slider';
...
export default function ProductDetail() {
  ...
  const initialPayment = (percent: number) => {
    const firstMount = Number(newPrice(productDetail?.price)) * percent / 100
    return firstMount.toFixed(2)
  }

  const restOfTheMount = (percent: number) => {
    const firstMount = initialPayment(percent)
    const rest = (Number(newPrice(productDetail?.price)) - Number(firstMount)) / 24
    ...
    <>
      <Container sx={{ maxWidth: '1500px!important' }}>
        ...
        <Container sx={{ display: 'flex', gap: '0.5rem', margin: '0!important', position: 'relative' }}>
          ...
          <Container sx={{ padding: '0px!important', display: 'flex', flexDirection: 'column', gap: 0.5, '@media (max-width: 768px)': { display: 'none' } }}>
            <Container
              component="img"
              alt={image}
              src={image || productDetail?.images[0]}
              sx={styles.mainImage}
            />
            <Box sx={styles.horizontalImagesBox}>
        ...
        <Grid container maxWidth="xl" paddingInline={3.5} marginBlock={2} sx={{ '@media (max-width): 900px': { placeItems: 'center' } }}>
          <Grid item md={6} xs={12} sx={styles.gridImagesContainer}>
                ...
                {!!productDetail?.images && productDetail?.images.map((image, idx) => (
                  <ImageListItem key={idx}>
                    <Container
                      ...
                      alt={image}
                      src={image}
                      sx={{ width: '100%', height: '100%', padding: '0px!important', objectFit: 'cover' }}
                    />
                  </ImageListItem>
                ))}
          ...
          <Grid item md={6} xs={12}>
            ...
            <Grid container>
              <Grid item xs={12} sm={6} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingLeft: '2rem', '@media (max-width: 600px)': { textAlign: 'center' } }}>
                ...
                <Container sx={{ '@media (max-width: 600px)': { padding: '0 auto' } }}>
                  {list.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', gap: '0.5rem', margin: '0.5rem 0'}}>
              ...
              <Grid item xs={12} sm={6}>
                <Card sx={{ paddingBlock: '0.5rem', borderRadius: '13px', maxWidth: '300px', height: '340px', margin: '0 auto' }}>
                  <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', textAlign: 'center' }}>
                    ...
                    <Typography variant='body2' color='black' sx={{ fontWeight: 'thin' }}>
                      ENGANCHE ${initialPayment(percent)} <br />
                      PAGO SEMANAL ${restOfTheMount(percent)}
                    </Typography>
                    <CustomizedSlider percent={percent} setPercent={setPercent} />
